  ...
  return (
    <div className="min-h-screen bg-gray-50 text-slate-900 pb-20 font-sans">
      <header className="sticky top-0 z-50 bg-white/95 backdrop-blur-md border-b border-gray-100 p-8 shadow-sm flex flex-col items-center">
        ...
        <h1 className="text-2xl font-black italic uppercase tracking-[0.25em] text-red-600 leading-none">{store.subtext}</h1>
      </header>
      <main className="max-w-4xl mx-auto p-4 space-y-6">
        <div className="relative pt-4">
          <Search className="absolute left-6 top-[62%] -translate-y-1/2 text-slate-300" size={24} />
          <input type="text" placeholder="Cerca marca o aroma..." className="w-full border-none pl-16 pr-8 py-6 rounded-[2.5rem] shadow-2xl shadow-gray-200/50 focus:ring-2 ring-red-500 outline-none text-lg font-bold" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} />
        </div>
        <div className="flex gap-2 overflow-x-auto pb-2 no-scrollbar px-1">
          {categories.map(cat => (
            <button key={cat} onClick={() => setActiveCategory(cat)} className={`flex items-center gap-2 px-6 py-4 rounded-full whitespace-nowrap font-black text-[10px] uppercase tracking-widest italic transition-all ${activeCategory === cat ? 'bg-red-600 text-white shadow-lg' : 'bg-white text-slate-500 border border-gray-100'}`}>
        ...
        <div className="flex gap-2 overflow-x-auto pb-2 no-scrollbar px-1">
          {uniqueFormats.map(formato => (
            <button key={formato} onClick={() => setActiveFormat(formato)} className={`flex items-center gap-2 px-5 py-3 rounded-2xl whitespace-nowrap font-black text-[10px] uppercase tracking-widest italic transition-all ${activeFormat === formato ? 'bg-slate-900 text-white shadow-lg' : 'bg-white text-slate-400 border border-gray-200 shadow-sm'}`}>
        ...
        <div className="flex gap-2 overflow-x-auto pb-4 no-scrollbar px-1">
          {uniqueBrands.map(brand => (
            <button key={brand} onClick={() => setActiveBrand(brand)} className={`px-6 py-3 rounded-2xl border font-black text-[10px] uppercase tracking-widest transition-all ${activeBrand === brand ? 'bg-red-600 border-red-600 text-white shadow-md' : 'bg-white border-gray-200 text-slate-400'}`}>
              {brand}
            </button>
          ))}
        </div>
        <div className="grid grid-cols-1 gap-6 pt-4">
          {filteredProducts.map(p => (
            <div key={p.id} className="bg-white p-6 rounded-[3.5rem] shadow-sm border border-gray-50 flex gap-6 items-start relative overflow-hidden group">
              {p.prezzo && <div className="absolute top-0 right-10 bg-red-600 text-white px-6 py-2 rounded-b-3xl font-black italic text-xl shadow-lg">€{parseFloat(p.prezzo).toFixed(2)}</div>}
              <div className="w-28 h-28 bg-gray-50 rounded-[2.5rem] overflow-hidden flex-shrink-0 border border-gray-100 p-2 group-hover:scale-105 transition-transform"><img src={p.image_url || 'https://via.placeholder.com/150'} className="w-full h-full object-contain rounded-2xl" /></div>
              <div className="flex-1 pt-2">
                <div className="inline-flex items-center gap-1 bg-slate-900 text-white px-3 py-1 rounded-xl text-[9px] font-black uppercase tracking-widest mb-3 italic"><Beaker size={10} /> {p.formato || 'Liquido'}</div>
                ...
                <div className="flex flex-wrap items-center gap-4">
                  <div className="inline-flex items-center gap-2 px-4 py-2 bg-slate-50 rounded-full text-slate-500 border border-slate-100 shadow-sm"><CategoryIcon cat={p.category} /><span className="text-[9px] font-black uppercase tracking-widest">{p.category}</span></div>
                  {p.nicotina && p.nicotina.length > 0 && (
                    <div className="flex gap-1 items-center"><span className="text-[9px] font-bold text-slate-300 uppercase mr-1">Nic:</span>
                      {p.nicotina.map(n => <span key={n} className="bg-white text-slate-800 px-3 py-1.5 rounded-xl text-[10px] font-black border border-gray-100 shadow-sm">{n}</span>)}
                    </div>
                  )}
                </div>
              </div>
      ...
      <footer className="bg-white border-t border-gray-200 mt-16 p-12 pb-24 text-center">
        <h2 className="text-4xl font-black italic uppercase tracking-tighter text-red-600 mb-2">{store.subtext}</h2>
