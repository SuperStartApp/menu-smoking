...
export default function MenuPage({ params }) {
  ...
  const filteredProducts = useMemo(() => {
    return allProducts.filter(p => {
      ...
      return matchSearch && matchCat && matchFormat && matchBrand;
    });
          ...
          <div className="flex gap-2 overflow-x-auto pb-1 no-scrollbar px-1">
            {uniqueBrands.map(brand => (
              <button key={brand} onClick={() => setActiveBrand(brand)}
                className={`flex items-center gap-2 px-5 py-2.5 rounded-2xl whitespace-nowrap font-black text-[9px] uppercase italic transition-all ${activeBrand === brand ? 'bg-red-600 border-red-600 text-white shadow-lg' : 'bg-white text-slate-400 border border-gray-200'}`}>
                <Tags size={12} /> {brand}
              </button>
            ))}
          </div>
        ...
        {totalPages > 1 && (
          <div className="flex items-center justify-center gap-4 py-8">
            <button 
              ...
              className={`p-4 rounded-full border-2 transition-all ${currentPage === 1 ? 'text-slate-200 border-slate-100' : 'text-red-600 border-red-100 hover:bg-red-50'}`}
            >
              <ChevronLeft size={24} />
            </button>
            ...
            <button 
              ...
              className={`p-4 rounded-full border-2 transition-all ${currentPage === totalPages ? 'text-slate-200 border-slate-100' : 'text-red-600 border-red-100 hover:bg-red-50'}`}
            >
      ...
      <footer className="bg-white border-t border-gray-100 mt-10 p-10 pb-24 text-center">
        <div className="max-w-4xl mx-auto space-y-6">
          ...
          <div className="grid grid-cols-2 gap-4 max-w-xs mx-auto">
            <a href={`tel:${store.phone}`} className="flex flex-col items-center p-6 border-2 border-gray-50 rounded-[2rem] bg-white"><Phone size={24} className="text-red-500 mb-2" /><span className="text-[10px] font-black uppercase text-slate-400 italic">Chiama</span></a>
            <a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(store.address || '')}`} target="_blank" className="flex flex-col items-center p-6 border-2 border-gray-50 rounded-[2rem] bg-white"><MapPin size={24} className="text-red-500 mb-2" /><span className="text-[10px] font-black uppercase text-slate-400 italic">Mappa</span></a>
          </div>
