  ...
  useEffect(() => {
    async function loadData() {
      ...
      if (storeData) {
        ...
        if (invData) {
          const products = invData.map((item: any) => item.vape_products).filter((p: any) => p !== null);
          products.sort((a: any, b: any) => a.brand.localeCompare(b.brand) || a.name.localeCompare(b.name));
      ...
      <footer className="bg-white border-t border-gray-200 mt-16 p-12 pb-24">
        <div className="max-w-4xl mx-auto space-y-12 text-center">
          ...
          <div className="grid grid-cols-2 gap-6">
            <a href={`tel:${store.phone}`} className="flex flex-col items-center p-8 border-2 border-gray-100 rounded-[3rem] hover:border-red-500 transition-all group bg-white shadow-sm">
              <Phone size={32} className="text-red-500 mb-3 group-hover:scale-110 transition-transform" />
              <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Chiama Ora</span>
            </a>
            <a href={`https://www.google.com/maps/search/?api=1&query=${encodeURIComponent(store.address || '')}`} target="_blank" className="flex flex-col items-center p-8 border-2 border-gray-100 rounded-[3rem] hover:border-red-500 transition-all group bg-white shadow-sm">
              <MapPin size={32} className="text-red-500 mb-3 group-hover:scale-110 transition-transform" />
              <span className="text-[10px] font-black uppercase tracking-widest text-slate-400">Mappa</span>
            </a>
          </div>
          {store.google_review_url && (
            <a href={store.google_review_url} target="_blank" className="flex items-center justify-center gap-5 bg-yellow-400 text-yellow-950 font-black p-8 rounded-[3.5rem] shadow-2xl shadow-yellow-100 hover:scale-[1.03] transition-all text-[11px] uppercase tracking-[0.2em] italic border-b-4 border-yellow-600">
